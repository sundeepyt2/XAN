...
// app/(app)/watch/[id]/WatchPageClient.tsx
// ✅ Client component — receives animeId, episodeStr, typeStr as props from Server Component.
// Avoids useSearchParams (which breaks on Cloudflare Pages edge runtime).
...
export function WatchPageClient({ animeId: animeIdStr, episodeStr, typeStr }: { animeId: string; episodeStr?: string; typeStr?: string }) {
  const animeId = parseInt(animeIdStr, 10);
  
  const router = useRouter();
  const episodeParam = episodeStr;
  ...
  const urlMode = typeStr === "dub" ? "dub" : null;
  ...
  const handleModeChange = useCallback(
    (newMode: "sub" | "dub") => {
      ...
      // ✅ Use window.location.search instead of useSearchParams (edge compat)
    ...
    [router, animeId, setPreferredMode],
  );
